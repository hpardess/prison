...
class Prisoner_model extends CI_Model {
    ...
    function get_by_id_with_joins($id){
        $this->db->select('`prisoner`.`id` AS `id`,`prisoner`.`marital_status_id` AS `marital_status_id`,`marital_status`.`status` AS `marital_status`,`prisoner`.`present_province_id` AS `present_province_id`,`present_province`.`name` AS `present_province`,`prisoner`.`present_district_id` AS `present_district_id`,`present_district`.`name` AS `present_district`,`prisoner`.`permanent_province_id` AS `permanent_province_id`,`permanent_province`.`name` AS `permanent_province`,`prisoner`.`permanent_district_id` AS `permanent_district_id`,`permanent_district`.`name` AS `permanent_district`,`prisoner`.`name` AS `name`,`prisoner`.`father_name` AS `father_name`,`prisoner`.`grand_father_name` AS `grand_father_name`,`prisoner`.`age` AS `age`,`prisoner`.`criminal_history` AS `criminal_history`,`prisoner`.`num_of_children` AS `num_of_children`,`prisoner`.`profile_pic` AS `profile_pic`');
        $this->db->from($this->tableName);
